...
// Constants for API
// Constants for API
// const USDT_VND_RATE = 25500; // Removed hardcoded rate
...
export const InvestmentDashboard: React.FC = () => {
  ...
  const [newBuyCurrency, setNewBuyCurrency] = useState<'VND' | 'USD'>('VND');

  // Settings & Global View
  const [exchangeRate, setExchangeRate] = useState(25500); // Default fallback
  const [viewCurrency, setViewCurrency] = useState<'VND' | 'USD'>('VND');
      ...
      if (asset.type === AssetType.CRYPTO) {
        try {
          ...
          if (response.ok) {
            ...
            newPrice = parseFloat(data.price) * exchangeRate;
          }
        } catch (error) {
  ...
  const handleAddAsset = async (e: React.FormEvent) => {
    ...
    const buyPriceVND = newBuyCurrency === 'USD'
      ? parseFloat(newBuyPrice) * exchangeRate
      : parseFloat(newBuyPrice);

    let currentPrice = buyPriceVND;
    ...
    try {
      if (newType === AssetType.CRYPTO) {
        ...
        if (response.ok) {
          const data = await response.json();
          currentPrice = parseFloat(data.price) * exchangeRate;
        }
      } else {
        // Fetch Stock Price
  ...
  const formatCurrency = (valVND: number) => {
    if (viewCurrency === 'USD') {
      return new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 2 }).format(valVND / exchangeRate);
    }
    return new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND', maximumFractionDigits: 0 }).format(valVND);
  };
        ...
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 md:gap-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto">
              {/* Currency Toggle */}
              <div className="bg-gray-100 p-1 rounded-lg flex shrink-0 mr-2">
                <button onClick={() => setViewCurrency('VND')} className={`px-3 py-1.5 rounded-md text-xs md:text-sm font-bold transition-all ${viewCurrency === 'VND' ? 'bg-white text-brand shadow-sm' : 'text-slate-400'}`}>🇻🇳 VND</button>
                <button onClick={() => setViewCurrency('USD')} className={`px-3 py-1.5 rounded-md text-xs md:text-sm font-bold transition-all ${viewCurrency === 'USD' ? 'bg-white text-brand shadow-sm' : 'text-slate-400'}`}>🇺🇸 USD</button>
              </div>
              ...
              <div className="bg-gray-100 p-1 rounded-lg flex shrink-0">
                {(['all', 'year', 'month', 'week', 'day'] as const).map((m) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-md text-xs md:text-sm font-medium transition-all capitalize whitespace-nowrap ${filterMode === m ? 'bg-white text-brand shadow-sm' : 'text-slate-500 hover:text-slate-900'
                      }`}
                  >
                    {m}
                  </button>
            ...
            <form onSubmit={handleAddAsset} className="space-y-5">
              ...
              <div>
                <div className="flex justify-between items-end mb-1.5">
                  <label className="text-xs font-semibold text-slate-500 uppercase">Buy Price ({newBuyCurrency})</label>
                  <div className="flex bg-gray-100 p-0.5 rounded-lg">
                    {(['VND', 'USD'] as const).map(c => (
                      <button
                        key={c}
                        type="button"
                        onClick={() => setNewBuyCurrency(c)}
                        className={`px-2 py-0.5 text-[10px] font-bold rounded ${newBuyCurrency === c ? 'bg-white shadow-sm text-brand' : 'text-slate-400 hover:text-slate-600'}`}
                      >
                        {c}
                      </button>
                    ))}
                ...
                <input
                  ...
                  className="w-full bg-gray-50 border border-gray-200 rounded-lg p-3 text-slate-900 focus:bg-white focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none"
                  placeholder={newBuyCurrency === 'VND' ? "e.g. 50000000" : "e.g. 2000"}
                />
              </div>
